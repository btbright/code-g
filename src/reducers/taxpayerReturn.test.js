...
describe("taxpayerReturn actions/reducer", () => {
  ...
  test("should add a state", () => {
    ...
    expect(result.residenceHistory[0].months).toEqual(
  ...
  test("should update a states", () => {
    ...
    expect(result.residenceHistory[0].months).toEqual(
      expect.arrayContaining([1, 4, 5, 6, 7])
    );
  ...
  test("should add a month to a state", () => {
    ...
    const result = taxpayerReturn(testState, testAction);
    expect(result.residenceHistory[0].months).toEqual(
      expect.arrayContaining([1, 4, 5, 6, 7, 9])
    );
  });
  ...
  test("should add all months on single state confirmation", () => {
    const testState = {
      residenceHistory: [{ stateAbbreviation: "AL", months: [] }]
    };
    const testAction = {
