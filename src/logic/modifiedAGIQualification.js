...
import { additionalFamilyMemberAmounts, federalPovertyLineLevels } from '../constants/federalPovertyLineData'
...
export function getModifiedAGIQualificationResult(taxpayerReturn){
	...
	const referenceFPL = getReferenceFPL(taxpayerReturn.taxYear, taxpayerReturn.numberOfPeopleInTaxHousehold)
...
export function isTaxpayerReturnComplete(taxpayerReturn){
	return !!taxpayerReturn.taxYear &&
				 ...
				 !!taxpayerReturn.residenceHistory && isTaxpayerReturnComplete.length !== 0
}
...
function parseField(source, destination){
	return fieldName => {
		const parseFunction = fieldTypes[fieldName] === 'integer' ? parseInt : parseFloat
...
export function calculateModifiedAGI(taxpayerReturn){
	...
	const individualTotal =  taxpayerReturn.AGI +
				 taxpayerReturn.taxExemptInterest +
				 taxpayerReturn.foreignEarnedIncome +
				 taxpayerReturn.socialSecurityBenefitsTotal -
				 taxpayerReturn.socialSecurityBenefitsTaxable;
	...
	const dependentsTotal = taxpayerReturn.dependents.reduce((total, dependent) => {
		return total +
					 dependent.AGI +
...
export function getReferenceFPL(taxYear, numberOfPeopleInTaxHousehold){
	...
	const householdSizeKey = numberOfPeopleInTaxHousehold <= householdMax ? numberOfPeopleInTaxHousehold.toString() : "8";
	...
	const additionalFamilyMemberAmount = additionalFamilyMemberAmounts[taxYearKey];

	//get the number of family members beyond the max in the data
	const numberOfAdditionalFamilyMembers = numberOfPeopleInTaxHousehold - householdMax;

	//to to the base, add the specified amount for each family member past the max
	return baseFPL + (additionalFamilyMemberAmount * numberOfAdditionalFamilyMembers);
}
