import {
  isTaxpayerReturnComplete,
  parseTaxpayerReturn,
  calculateModifiedAGI,
  getReferenceFPL,
  calculatePercentageOfFPL,
  getModifiedAGIQualificationResult
} from "./modifiedAGIQualification";
...
describe("ModifiedAGIQualification logic", () => {
  ...
  describe("parseTaxpayerReturn", () => {
    test("should be parse fields properly", () => {
      ...
      expect(getObjectDiff(parsedReturnReference, parsedReturn)).toEqual(
  ...
  describe("getModifiedAGIQualificationResult", () => {
    test("should handle simple example", () => {
      ...
      const {
        fplPercentage,
        isTaxpayerQualified
      } = getModifiedAGIQualificationResult(taxpayerReturn);

      expect({ fplPercentage, isTaxpayerQualified }).toEqual({
        fplPercentage: 42.19,
        isTaxpayerQualified: true
      });
    });
    ...
    test("should handle tests", () => {
      returnTests.forEach(taxpayerReturn => {
        ...
        const {
          fplPercentage,
          isTaxpayerQualified
        } = getModifiedAGIQualificationResult(returnTest);
        const diff = getObjectDiff(result, {
          fplPercentage,
          isTaxpayerQualified
        });
