...
export default store => next => action => {
	...
	if (action.type !== types.UPDATE_STEP || action.increment < 0 || action.step < state.ui.step) return next(action)
	...
	const invalidlyMissingValueFieldNames = requiredFields.reduce((invalidFieldNames,field) => {
		if (state.taxpayerReturn[field.fieldName] === ""){
	...
	if (invalidlyMissingValueFieldNames.length !== 0){
		store.dispatch({
			...
			errors: invalidlyMissingValueFieldNames.map(fieldName => ({fieldName, errorText: "Field is required"}))
		})
		return
	}
